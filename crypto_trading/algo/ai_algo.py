...
class AIAlgo(AlgoIf):
    ...
    def process(self, current_value, values, currency, indicator_signals):
        ...
        # Use last `NUM_PRICE_FEATURES` from values, or fewer if not available, then pad.
        # The model expects a fixed size, so padding or a different strategy for handling
        # insufficient history is important.

        # We need NUM_PRICE_FEATURES - 1 from historical `values` because current_value is the last one.
        ...
        if len(values) >= num_historical_needed:
            ...
            # Insufficient historical data, pad with the oldest available value or current_value if values is empty
            ...
            logging.warning(
                f"AIAlgo: Not enough historical price data ({len(values)} points) for {self.NUM_PRICE_FEATURES} features. "
